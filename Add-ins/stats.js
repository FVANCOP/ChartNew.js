...
function PSbasic(data) {
  ...
  for(var i=0;i<data["length"];i++) {
      if (!isStat(data[i].value)) { (data.stats.count_all)++; }
      else data.stats.data_with_stats=true;
      
      if(typeof data[i].value=="undefined") {
        (data.stats.count_missing)++;
      } else if (isStat(data[i].value)) {   
      } else {
  ...
  for(i=0;i<data.length;i++) {
    if(typeof data[i].value!="undefined" && !isStat(data[i].value)) { 
        orderStat[cnt]={ val: 1*data[i].value, one : 1 };
        cnt++;
...
function LSbasic(data) {
  ...
  for(var i=0;i<data.datasets["length"];i++)
  {
    ...
    for(var j=0;j<data.datasets[i].data["length"];j++) {
      ...
      } else if (isStat(data.datasets[i].data[j])) { 
      } else {
        (data.stats.count_not_missing)++;
  ...
  for(i=0;i<data.datasets["length"];i++) {
    for(j=0;j<data.datasets[i].data["length"];j++) {
      if(typeof data.datasets[i].data[j]!="undefined" && !isStat(data.datasets[i].data[j].value)) { 
        orderStat[cnt]={ val: 1*data.datasets[i].data[j], dataset : i, col : j, one : 1 };
        cnt++;
      }
    }
  }
...
function Pstats(orderStat,secVar) {
  ...
  for(i=1;i<orderStat.length;i++) {
    if(orderStat[i][secVar]==orderStat[deb][secVar])fin++;
      ...
      result[result.length]={ secvalue: orderStat[deb][secVar], res : P2stats(deb,fin,orderStat) };
      fin++;deb=fin;
    }
  }
  result[result.length]={ secvalue: orderStat[deb][secVar], res : P2stats(deb,fin,orderStat) };
    ...
    if (typeof data.datasets=='undefined') { // Pie structure;
      for(i=0;i<data.length;i++)
      {
        ...
        if(isTemplate(data[i].value)) {
          data[i].value=tmplStat(data[i].value, { V1:1});
        }
      ...
      {
        for(var j=0;j<data.datasets[i].data["length"];j++) {
          if (isStat(data.datasets[i].data[j])) { data.datasets[i].data[j]=replace_Stats_In(data.datasets[i].data[j],data,i,j);}
          // templates ?
          if(isTemplate(data.datasets[i].data[j])) {
            data.datasets[i].data[j]=tmplStat(data.datasets[i].data[j], { V1:1});
          }
        }
      }
    }
  ...
  if (typeof data.datasets=='undefined') { // Pie structure;
      for(i=0;i<data.length;i++)
      {
        ...
        if(isTemplate(data[i].title)) {
          data[i].title=tmplStat(data[i].title, { V1:1});
        }
      }
  } else { // line structure;
      for(var i=0;i<data.datasets["length"];i++)
      {
        if (isStat(data.datasets[i].title)) { data.datasets[i].title=replace_Stats_In(data.datasets[i].title,data,i,-1);}
        // templates ?
        if(isTemplate(data.datasets[i].title)) {
          data.datasets[i].title=tmplStat(data.datasets[i].title, { V1:1});
        }
      }
  }
    ...
    if(typeof obj[attrname]=="object") {
      ...
      if(!(attrname=="annotateLabel" || attrname=="inGraphDataTmpl" || attrname=="scaleLabel")) {
        if(isTemplate(obj[attrname])) {
          obj[attrname]=tmplStat(obj[attrname], { V1:1});
        }
      } 
    }
  }
...
function convertStat(statval,data,dataset,coldata) {
  ...
  if(typeof data.stats[statval.toLowerCase()] != "undefined" && typeof data.stats[statval.toLowerCase()] != "object"){ 
    ...
    } else { vdataset=Math.max(1*dataset,0);}
    ...
    } else { vdataset=Math.max(1*coldata,0);}
    if(typeof data.datasets=="object") {
